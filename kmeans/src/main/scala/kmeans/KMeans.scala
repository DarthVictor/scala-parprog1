...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val meansGroup = points.groupBy(point => findClosest(point, means))
    means.seq.foldLeft(meansGroup) ((meansGroup: GenMap[Point, GenSeq[Point]], mean: Point) =>{
      if(!meansGroup.contains(mean)){
        meansGroup + (mean -> GenSeq())
      }
      else{
        meansGroup
      }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    (oldMeans zip newMeans).find( p => {
      val (oldMean, newMean) = p
